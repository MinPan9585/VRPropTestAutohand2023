...
namespace MRSpace.FrameWork.SystemProps
{
    ...
    public class PatchMesh : MonoBehaviour
    {
        ...
        public void CreateBrushMesh(Material _matrial)
        {
            mesh = GetComponent<MeshFilter>().mesh;
            GetComponent<MeshRenderer>().sharedMaterial = new Material(_matrial);
        }

        public void AddMesh(Vector3 point, Vector3 point1)
        {
            if (s != Vector3.zero)
            {


                AddLine(mesh, MakeQuad(s, sl, point, point1, firstQuad));
                firstQuad = false;
            }
            s = point;
            sl = point1;
        ...
        Vector3[] MakeQuad(Vector3 s, Vector3 hs, Vector3 er, Vector3 e, bool all)
        {
